      ...
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Manage, create, and moderate job posts.
          </p>
        </div>
        <button
          onClick={() => {
        ...
        {selectedFilter === "pending" ? (
          ...
          <>
            {pendingJobs.map((job, idx) => (
              <Card
                key={job.id || idx}
                title={`Review Request: ${job.title || "Unknown"}`}
                className={job.risk === "High" ? "border-red-200" : ""}
              >
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className="w-full md:w-72 flex flex-col space-y-4">
                    {job.risk && (
                      <div
                        className={`p-4 rounded-lg flex items-start space-x-3 ${job.risk === "High" ? "bg-red-50 border border-red-100" : "bg-yellow-50 border border-yellow-100"}`}
                      >
                        <ShieldAlert
                          className={`h-5 w-5 mt-0.5 ${job.risk === "High" ? "text-red-500" : "text-yellow-500"}`}
                        />
                        <div>
                          <h4
                            className={`text-sm font-bold ${job.risk === "High" ? "text-red-800" : "text-yellow-800"}`}
                          >
                            System Flag: {job.risk} Risk
                          </h4>
                          <p
                            className={`text-xs mt-1 ${job.risk === "High" ? "text-red-600" : "text-yellow-700"}`}
                          >
                            {job.reason || "Flagged for review"}
                          </p>
                        </div>
                      </div>
                    )}
              ...
              <tbody>
                {allJobs.map((job, idx) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <Badge
                        className={`text-xs ${job.status === "active" ? "bg-green-100 text-green-800" : "bg-slate-100 text-slate-800"}`}
                      >
                        {job.status || "Active"}
                      </Badge>
                    </td>
