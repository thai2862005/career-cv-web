  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (jobsResult.success && jobsResult.data) {
        const jobsData = Array.isArray(jobsResult.data)
          ? jobsResult.data
          : Array.isArray(jobsResult.data?.data)
            ? jobsResult.data.data
            : Array.isArray(jobsResult.data?.items)
              ? jobsResult.data.items
              : Array.isArray(jobsResult.data?.content)
                ? jobsResult.data.content
                : [];
        ...
        const usersData = Array.isArray(usersResult.data)
          ...
          : Array.isArray(usersResult.data?.data)
            ...
            : Array.isArray(usersResult.data?.items)
              ? usersResult.data.items
              : Array.isArray(usersResult.data?.content)
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-slate-500 mt-1">
          Platform overview, metrics and recent system activities.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, idx) => (
          <Card
            key={idx}
            className="flex items-center space-x-4 p-2 transition-transform hover:-translate-y-1 duration-300"
          >
                  ...
                  {pendingModerations.map((mod, idx) => (
                    <tr key={idx} className="hover:bg-slate-50">
                      <td className="px-4 py-3 font-medium text-slate-700">
                        {mod.company?.name || "Unknown Company"}
                      </td>
                      <td className="px-4 py-3 text-slate-600">{mod.title}</td>
                      <td className="px-4 py-3">
                        <Badge variant="yellow">{mod.reason}</Badge>
                      </td>
                      <td className="px-4 py-3">
                        <Badge variant={mod.risk === "High" ? "red" : "yellow"}>
                          {mod.risk}
                        </Badge>
                      </td>
                    </tr>
          ...
          <div className="space-y-4">
            {recentActivity.map((log, idx) => (
              <div key={idx} className="flex items-start space-x-3">
                <div
                  className={`mt-0.5 w-2 h-2 rounded-full flex-shrink-0 ${log.type === "system" ? "bg-blue-400" : log.type === "billing" ? "bg-green-400" : "bg-red-400"}`}
                ></div>
