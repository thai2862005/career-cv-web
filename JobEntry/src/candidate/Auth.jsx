      ...
      if (isLogin) {
        ...
        if (result.success) {
          ...
          const userRole = String(
            user?.role || user?.Role || user?.roleName || "JOB_SEEKER",
          ).toUpperCase();
  ...
  return (
    <div className="min-h-screen bg-white flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-slate-900 justify-center items-center overflow-hidden">
        ...
        <button
          onClick={() => navigate("/")}
          className="relative z-10 max-w-md px-8 text-center hover:opacity-80 transition-opacity duration-200 cursor-pointer"
        >
          <div className="p-4 rounded-xl bg-white/10 backdrop-blur-md inline-flex items-center justify-center mb-8 border border-white/20 hover:bg-white/20 transition-colors">
            <Briefcase className="h-10 w-10 text-white" />
          </div>
          <h1 className="text-4xl font-bold text-white mb-6 leading-tight">
            Elevate your career with JobEntry
          </h1>
          <p className="text-lg text-blue-100">
            Join a network of over 10,000 professionals and get discovered by
            top-tier organizations globally.
          </p>
        </button>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 md:p-24 bg-white">
        <div className="w-full max-w-md">
          ...
          <Link
            to="/"
            className="flex items-center gap-2 mb-12 lg:hidden justify-center"
          >
          ...
          <form className="space-y-4" onSubmit={handleAuth}>
              ...
              <div className="flex flex-col space-y-2">
                ...
                {role && (
                  <p className="text-xs text-slate-500">
                    Selected: {ROLES[role]?.display} • Role ID:{" "}
                    {ROLES[role]?.id}
                  </p>
                )}
              </div>
            ...
            {isLogin && (
              <div className="flex justify-between items-center pb-2">
                <label className="flex items-center text-sm text-slate-600">
                  <input
                    type="checkbox"
                    className="rounded border-slate-300 text-[var(--color-primary)] focus:ring-[var(--color-primary)] mr-2 w-4 h-4"
                  />
                  Remember me
                </label>
                <a
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                type="button"
                className="flex justify-center items-center py-2.5 border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors"
              >
              ...
              <button
                type="button"
                className="flex justify-center items-center py-2.5 border border-slate-300 rounded-lg hover:bg-slate-50 transition-colors text-slate-800 font-medium"
              >
                <svg
