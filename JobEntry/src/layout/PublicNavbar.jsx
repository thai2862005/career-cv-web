...
export const PublicNavbar = () => {
  ...
  return (
    <nav
      className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${isScrolled ? "bg-white shadow-md py-3" : "bg-transparent py-5"}`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          <Link to="/" className="flex items-center gap-2 group">
            <div
              className={`p-1.5 rounded-lg transition-colors ${isScrolled ? "bg-[var(--color-primary-light)] text-[var(--color-primary)]" : "bg-[var(--color-primary)] text-white"}`}
            >
              <Briefcase className="h-6 w-6" />
            </div>
            <span
              className={`text-xl font-bold tracking-tight ${isScrolled ? "text-slate-800" : isHomePage ? "text-white" : "text-slate-800"}`}
            >
              JobEntry
            </span>
          </Link>
            ...
            {isAuthenticated ? (
              <>
                <Link
                  ...
                  className={`text-sm font-medium ${isScrolled ? "text-[var(--color-primary)]" : isHomePage ? "text-white font-bold" : "text-[var(--color-primary)] font-bold"}`}
                >
                  Dashboard
                </Link>
                <button
                  ...
                  className={`text-sm font-medium ${isScrolled ? "text-slate-600 hover:text-slate-900" : isHomePage ? "text-slate-100 hover:text-white" : "text-slate-600 hover:text-slate-900"}`}
                >
                  Log out
                </button>
              </>
            ...
            <div className="border-t border-slate-100 pt-4 mt-2 grid gap-2">
              {isAuthenticated ? (
                ...
                <>
                  <Link
                    to="/login"
                    className="block px-3 py-2.5 rounded-md text-center text-base font-medium border border-slate-200 text-slate-700 hover:bg-slate-50"
                  >
                    Log in
                  </Link>
                  <Link
